    ...
    <header className="bg-[#104B64] w-full h-[80px] flex items-center font-poppins relative z-50">
      <nav className="w-full max-w-[1320px] mx-auto px-6 md:px-8 lg:px-16 flex items-center justify-between">
        ...
        <div className="hidden lg:flex gap-4 items-center">
          <BtnCadastrar
            href="/login"
            className="min-w-[150px] border border-white text-white bg-transparent
             hover:bg-white hover:text-[#104B64]"
          >
            Entrar
          </BtnCadastrar>
          <BtnCadastrar
      ...
      {isMenuOpen && (
        <div className="lg:hidden absolute top-[80px] left-0 w-full bg-[#104B64] flex flex-col items-center gap-6 py-6 text-white z-40 font-poppins text-base px-4">
          {navLinks.map(({ href, label }) => (
            <Link key={href} href={href} onClick={() => setIsMenuOpen(false)} className="hover:text-[#0990BA] transition">
              {label}
            </Link>
          ))}
