  ...
  return (
    <>
      ...
      <section className="w-full bg-[#f6f2fa] py-20">
        <div className="max-w-[1220px] mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-16">
          {cards.map((card, idx) => (
            <article
              key={idx}
              className="bg-[#104B64] text-white flex flex-col items-center p-9 rounded-lg w-full"
            >
              <img
                src={`/${card.img}`}
                alt={card.title}
                className="w-full h-auto max-h-[150px] object-contain mb-4 rounded-md"
              />
      ...
      <section className="w-full bg-white py-20">
        <div className="max-w-[1220px] mx-auto px-4 text-center">
          ...
          <p className="text-lg font-poppins text-black-700 mb-8">
            Escolha o seu formulário e comece a editar agora
          </p>

          <div className="relative w-full max-w-9xl mx-auto mb-10">
            <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 w-5 h-10" />
            <input
              type="text"
              placeholder="Procure o seu formulário aqui"
              className="w-full pl-10 pr-4 py-3 border-2 border-[#DFDFDF] rounded-lg placeholder-gray-400 focus:outline-none focus:border-[#104B64] shadow-md"
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-left">
            <ul className="space-y-2">
              {[
                ...
                <li key={i}><a href="#" className="hover:text-[#0990BA] transition">{item}</a></li>
              ))}
            </ul>
      ...
      <section className="w-full bg-[#104B64]/10 py-20">
        <div className="max-w-[1320px] mx-auto px-4 flex flex-col md:flex-row items-center gap-10">
          <div className="w-full md:w-1/2 md:pr-10 -ml-84 mb-10 md:mb-0">
            <img
              src="/Rectangle.png"
